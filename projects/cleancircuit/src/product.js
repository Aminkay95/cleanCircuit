...
function createProductRouter(store, authorize) {
  ...
  router.post("/jobs/:id/quote/send", emailTransition(store,"quote.sent",job=>{job.quote.status="sent";return job;},job=>({to:job.customer.email,subject:`Quote ${job.quote.number} from ${process.env.PRODUCT_NAME||"CleanCircuit"}`,html:`<p>Your service quote is ready.</p>`,idempotencyKey:`quote-${job.id}`})));
  ...
  router.post("/jobs/:id/deposit/request", async(req,res,next)=>{try{const job=(await store.all()).find(item=>item.id===req.params.id);if(!job)return res.status(404).json({error:"Job not found"});if(job.contract.status!=="signed")throw conflict("Contract must be signed before requesting a deposit");const payment=await initializeDeposit({email:job.customer.email,amountCents:job.service.depositCents,reference:`deposit_${job.id}`,callbackUrl:`${process.env.PUBLIC_BASE_URL||"http://localhost:3001"}/product`});const updated=await store.transition(job.id,"deposit.link_created",current=>{current.deposit={status:"requested",provider:payment.provider,checkoutUrl:payment.authorizationUrl,reference:payment.reference};return current;});res.json(updated);}catch(error){next(error);}});
